...
async def ending(callback: CallbackQuery, state: FSMContext, bot : Bot):
    await callback.answer()
    try:
        res = datetime.datetime.strptime(callback.data, '%H:%M')
        final_res = res.strftime('%H:%M')
        ...
        notif_query = '''
                      SELECT booker_id FROM user_booking_data
                      '''
